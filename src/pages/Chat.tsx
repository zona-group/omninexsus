...
function MarkdownText({ text }: { text: string }) {
  // Very lightweight markdown: code blocks, inline code, bold
          ...
          <div className="ml-auto flex items-center gap-3">
            {!subscribed && (
              <div className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-full border ${
                ...
                {remaining === 0
                  ? 'Free limit reached'
                  : `${remaining}/${FREE_MESSAGE_LIMIT} free messages`}
              </div>
            )}
            {subscribed && (
        ...
        <div className="max-w-4xl mx-auto px-4 py-6 flex flex-col gap-6">
          ...
          {loading && (
            <div className="flex gap-3">
              <div className="flex-shrink-0 w-8 h-8 rounded-full bg-secondary border border-border/50 flex items-center justify-center text-lg">
                {currentMode.icon}
              </div>
        ...
        <div className="max-w-4xl mx-auto px-4 py-4">
          {!subscribed && remaining === 0 ? (
            <div className="flex items-center justify-between bg-gradient-to-r from-indigo-500/10 to-purple-600/10 border border-indigo-500/30 rounded-xl px-4 py-3">
              ...
              <Button
                size="sm"
                className="bg-gradient-to-r from-indigo-500 to-purple-600 ml-4"
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription className="text-center pt-2">
              You've used all {FREE_MESSAGE_LIMIT} free messages. Unlock unlimited AI assistance.
            </DialogDescription>
          </DialogHeader>
